...
/* Display tiers, borrowed from the YYSF week-tier palette: the color a
 * badge wears once it is UNLOCKED (wall row + toast). Locked rows never
 * hint their tier, and green (cheated) overrides everything. Assignments
 * are generated by wrapped_dev/tiers.py from per-ladder thresholds -
 * regenerate rather than hand-editing 345 rows. */
enum ach_tier_id { TIER_GOLD = 0, TIER_ICE, TIER_VIO, TIER_EMBER,
                   TIER_OMEGA, TIER_PINK, TIER_N };
...
struct ach_def {
    ...
    unsigned char tier;    /* TIER_*; rows that say nothing are gold */
};
...
/* current tally of one week, for live tier-crossing checks (achwatch) */
static unsigned __attribute__((unused)) at_week_secs_of(long wk)
{
    if (at_week_n && at_weeks[at_week_n - 1].wk == wk)
        return at_weeks[at_week_n - 1].secs;
    for (int i = 0; i < at_week_n; i++)
        if (at_weeks[i].wk == wk)
            return at_weeks[i].secs;
    return 0;
}
...
/* wall display order: identity in ladder mode; in date mode the unlocked
 * rows come first, newest unlock first ("before records" ones last among
 * them), with the locked remainder keeping table order below */
static int  ach_ord[sizeof(ach_table)/sizeof(ach_table[0])];
static bool ach_sort_bydate;

static void ach_build_order(void)
{
    int n = 0;
    if (ach_sort_bydate) {
        for (int i = 0; i < N_ACH; i++) {
            if (!ach_unlocked(i))
                continue;
            /* insertion by when desc; equal dates keep table order */
            int j = n++;
            while (j > 0 && ach_when[ach_ord[j - 1]] < ach_when[i]) {
                ach_ord[j] = ach_ord[j - 1];
                j--;
            }
            ach_ord[j] = i;
        }
    ...
    for (int i = 0; i < N_ACH; i++)
        if (!ach_sort_bydate || !ach_unlocked(i))
            ach_ord[n++] = i;
}
...
/* one metal per display tier; index 0 repeats ach_th so lookups stay
 * branch-free. TIER_PINK only ever renders on a build that carries the
 * personal row. */
static const unsigned ach_tier_col[TIER_N] = {
    COL_GOLD, COL_ICE, COL_VIO, COL_EMBER, COL_OMEGA, COL_PINK
};
static const unsigned ach_tier_hi[TIER_N] = {
    COL_GOLD_HI, COL_ICE_HI, COL_VIO_HI, COL_EMBER_HI, COL_OMEGA_HI,
    COL_PINK_HI
};
static const struct theme ach_tier_th[TIER_N] = {
    { LCD_RGBPACK(52, 38, 12), LCD_RGBPACK(18, 11,  6),
      LCD_RGBPACK(255,244,200), LCD_RGBPACK(255,196, 70), COL_GOLD  },
    { LCD_RGBPACK(10, 40, 54), LCD_RGBPACK( 4, 12, 18),
      LCD_RGBPACK(228,250,255), LCD_RGBPACK(115,215,250), COL_ICE   },
    { LCD_RGBPACK(38, 22, 56), LCD_RGBPACK(12,  7, 18),
      LCD_RGBPACK(243,232,255), LCD_RGBPACK(195,140,250), COL_VIO   },
    { LCD_RGBPACK(56, 24, 12), LCD_RGBPACK(18,  7,  4),
      LCD_RGBPACK(255,235,224), LCD_RGBPACK(250,130, 95), COL_EMBER },
    { LCD_RGBPACK(42, 46, 52), LCD_RGBPACK(13, 15, 18),
      LCD_RGBPACK(255,255,255), LCD_RGBPACK(205,240,255), COL_OMEGA },
    { LCD_RGBPACK(54, 18, 38), LCD_RGBPACK(18,  5, 12),
      LCD_RGBPACK(255,230,243), LCD_RGBPACK(250,155,205), COL_PINK  },
};

/* effective display tier of a row, clamped for safety */
static int ach_tier(int i)
{
    int t = ach_table[i].tier;
    return (t >= 0 && t < TIER_N) ? t : TIER_GOLD;
}
...
/* tier >= 0 paints that tier's metal; tier < 0 is the cheater's green */
static void ach_toast(const char *name, int tier, const char *kick)
{
    const int FR = 8;
    ...
    rb->cpu_boost(true);              /* PP can't animate this at idle */
#endif
...
#define TROW(y) (fb + (ptrdiff_t)(y) * stride)
    ...
    if (tier < 0)
        ...
        toast_paint(&ach_tier_th[tier], name,
                    ach_tier_col[tier], ach_tier_hi[tier], kick);
    for (int y = 0; y < TOAST_H; y++)
...
static void ach_draw_row(int y, int i, bool selected)
{
    ...
    if (ach_vis[i] == AV_DONE) {
        ...
        draw_disc(th, 17, cy, 8,
                  green ? COL_CHEAT    : ach_tier_col[t], 256);
        draw_disc(th, 17, cy, 4,
                  green ? COL_CHEAT_HI : ach_tier_hi[t], 256);
    } else if (pers) {
        /* ever so slightly special: a pink ring among the gold */
        draw_disc(th, 17, cy, 8, COL_PINK, 140);
    ...
    unsigned ncol = green                   ? COL_CHEAT_HI
                  : (ach_vis[i] == AV_DONE) ? ach_tier_hi[ach_tier(i)]
                  : pers                    ? COL_PINK_HI
    ...
    if (rtxt[0]) {
        rb->lcd_set_foreground(new_get(i)
                                   ? (green ? COL_CHEAT
                                            : ach_tier_col[ach_tier(i)])
                                   : COL_TEXT_DIM);
...
static void ach_draw_screen(int cursor, int top, int done, int newn)
{
...
#ifdef HAVE_ADJUSTABLE_CPU_FREQ
    rb->cpu_boost(true);       /* full-screen gradient + rows per frame */
#endif
    fill_gradient(th);
    if (cheat_rigged())
    ...
    for (int r = 0; r < ACH_ROWS; r++) {
        int i = top + r;
        if (i >= N_ACH) break;
        ach_draw_row(ACH_Y0 + r * ACH_RH, ach_ord[i], i == cursor);
    }

    /* footer: description + live progress bar for the selected row */
    int ci = ach_ord[cursor];
    ...
    const char *desc = (ach_vis[ci] == AV_HIDDEN)
                         ? "keep listening to reveal this one"
                         : (ach_table[ci].metric == AM_F_CHEAT
                            && !ach_unlocked(ci))
                         ? "don't even try - the iPod will know"
                         : ach_table[ci].desc;
    char fb[64];
    fit_text(desc, SCREEN_W - 20, fb, sizeof fb);
    center_text(fy, fb, (ach_vis[ci] != AV_DONE) ? COL_TEXT_DIM
                        : ach_is_green(ci) ? COL_CHEAT
                                           : ach_tier_col[ach_tier(ci)]);

    if (ach_vis[ci] == AV_DONE) {
        /* a finished badge doesn't need a full bar - it gets its date */
        char db[48];
        unsigned long w = ach_when[ci];
        if (w > MIN_VALID_TS) {
            int dy, dm, dd;
        ...
        long v = am_val[ach_table[ci].metric];
        long t = ach_threshold(ci);
        if (v > t) v = t;
        if (v < 0) v = 0;
...
/* ---- the options behind a held MENU ---- */

/* accidental-press insurance: nothing in this menu truly deletes, it
 * benches the file as <path>.bak (one slot, newest wins). The permanent
 * delete lives in the watcher's menu, where nobody lands by accident. */
static bool spun_bench(const char *path)
{
    char bak[64];
    rb->snprintf(bak, sizeof bak, "%s.bak", path);
    rb->remove(bak);
    return rb->rename(path, bak) == 0;
}

/* the true delete: shred every .bak the options menu benched */
static int spun_purge_backups(void) __attribute__((unused));
static int spun_purge_backups(void)
{
    static const char * const base[] = {
        PLAYBACK_LOG_PATH, PBL_SIG_PATH, CHEAT_DAT_PATH,
        ACH_SEEN_PATH, ACH_TOAST_FLAG, DBMAP_PATH,
    };
    char p[64];
    int n = 0;
    for (unsigned i = 0; i < ARRAYLEN(base); i++) {
        rb->snprintf(p, sizeof p, "%s.bak", base[i]);
        if (rb->remove(p) == 0)
            n++;
    }
    for (int k = 1; ; k++) {   /* rotation is contiguous, stop at the gap */
        rb->snprintf(p, sizeof p, "%s_%04d.log.bak", PLAYBACK_LOG_STEM, k);
        if (rb->remove(p) != 0)
            break;
        n++;
    }
...
/* forget dates, gold, NEW markers and the toast counters; the badges
 * themselves recompute from the log on the spot */
static bool ach_wipe_progress(void)
{
    static const char *lines[] = {
        "Delete badge progress?",
        "Unlock dates, gold and toasts",
        "reset - a .bak copy is kept.",
    };
    const struct text_message msg = { lines, 3 };
    if (rb->gui_syncyesno_run(&msg, NULL, NULL) != YESNO_YES)
        return false;
    spun_bench(ACH_SEEN_PATH);
    spun_bench(ACH_TOAST_FLAG);
    rb->memset(ach_seen, 0, sizeof(ach_seen));
    rb->memset(ach_gold, 0, sizeof(ach_gold));
    rb->memset(ach_when, 0, sizeof(ach_when));
    rb->memset(ach_new, 0, sizeof(ach_new));
    ...
    ach_toast_count = 0;
    ach_toast_burst = false;
    rb->splash(HZ * 2, "Badge progress deleted");
    return true;
}

/* the sanctioned full amnesty, from the couch instead of a USB cable:
 * logs + sig + verdict + badges go together (deleting only part of that
 * set is what the cheat detection exists to catch) */
static bool ach_start_over(void)
{
    static const char *l1[] = {
        "Start over completely?",
        "Erases the whole listening",
        "history, verdict and badges.",
    };
    const struct text_message m1 = { l1, 3 };
    if (rb->gui_syncyesno_run(&m1, NULL, NULL) != YESNO_YES)
        return false;
    static const char *l2[] = {
        "Last chance - really start over?",
        "The old files stay as .bak",
        "copies until you purge them.",
    };
    const struct text_message m2 = { l2, 3 };
    if (rb->gui_syncyesno_run(&m2, NULL, NULL) != YESNO_YES)
        return false;

    rb->add_playbacklog(NULL);        /* buffered lines out first */
    spun_bench(PLAYBACK_LOG_PATH);
    for (int n = 1; ; n++) {
        char p[48];
        rb->snprintf(p, sizeof p, "%s_%04d.log", PLAYBACK_LOG_STEM, n);
        if (!spun_bench(p))
            break;
    }
    spun_bench(PBL_SIG_PATH);
    spun_bench(CHEAT_DAT_PATH);
    spun_bench(ACH_SEEN_PATH);
    spun_bench(ACH_TOAST_FLAG);
    spun_bench(DBMAP_PATH);
    /* hidden-card picks survive: taste, not history */

    /* the core's in-RAM signing chain still describes the old bytes;
     * the flag makes it re-bless from scratch at the next boot and keeps
     * every verifier from passing a verdict until then */
    int fd = rb->open(SPUN_RESET_FLAG, O_WRONLY | O_CREAT | O_TRUNC, 0666);
    if (fd >= 0)
        rb->close(fd);
    ...
    ach_toast_count = 0;
    ach_toast_burst = false;
    rb->splash(HZ * 3, "Everything forgotten - restart to finish");
    return true;
}

static void ach_options(int *done, int *newn)
{
    MENUITEM_STRINGLIST(menu, "Achievements", NULL,
                        "Sort by unlock date", "Sort by ladder",
                        "Delete badge progress", "Start over completely");
    int sel = ach_sort_bydate ? 1 : 0;
    switch (rb->do_menu(&menu, &sel, NULL, false)) {
    case 0: ach_sort_bydate = true;  break;
    case 1: ach_sort_bydate = false; break;
    case 2:
        if (ach_wipe_progress()) {
            /* everything still earned re-stamps as fresh gold and NEW */
            if (ach_record_unlocks())
                ach_seen_save();
            *done = ach_classify();
            *newn = 0;
            for (int i = 0; i < N_ACH; i++)
...
static int ach_list_ui(void)
{
    ...
    for (;;) {
        ...
        if (base == BUTTON_SCROLL_FWD) {
            ...
            /* hop to the start of the next ladder (a page, when the
             * date sort has dissolved the ladders) */
            int prev = cursor;
            if (ach_sort_bydate) {
                cursor += ACH_ROWS;
                if (cursor > N_ACH - 1) cursor = N_ACH - 1;
            ...
            if (newn > 0) {
                for (int k = 1; k <= N_ACH; k++) {
                    ...
                    if (new_get(ach_ord[i])) { cursor = i; break; }
                }
            }
        } else if (base == BUTTON_PLAY) {
            save_card();
        } else if (btn == BUTTON_MENU) {
            continue;              /* decide on the repeat or the release */
        } else if (btn == (BUTTON_MENU | BUTTON_REPEAT)) {
            ach_options(&done, &newn);
            if (ach_reset_ran) {
                rb->lcd_setfont(FONT_SYSFIXED);
                return PLUGIN_OK;
            }
            rb->button_clear_queue();
        } else if (btn == (BUTTON_MENU | BUTTON_REL)) {
            break;                 /* a tapped MENU still just leaves */
        } else {
            continue;
        }
